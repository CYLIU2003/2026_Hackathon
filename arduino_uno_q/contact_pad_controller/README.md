...
| コマンド | 意味 |
|---|---|
| `RELEASE` | ダッシュボード Demo Mode 用。模擬入力で安全リリース動作を要求 |
| `STOP` | ダッシュボード Demo Mode 用。模擬入力を解除して閉状態へ戻す |
| `TEST` | ダッシュボード Demo Mode 用。デモ用テスト動作を要求 |
...
`RELEASE` と `TEST` は `ERROR_SAFE` を解除しません。エラー復帰には必ず
`RESET` を送信します。
