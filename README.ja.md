...
- モデルパス: models/yolo_bear.pt
- 推奨解像度: 640x480 または 320x240
- 推奨FourCC: まずMJPG、失敗時にYUYV
- 失敗時の挙動: ai_bear_approaching=false
```

`models/yolo_bear.pt` が無い場合、`AI_MODEL_LOAD_ERROR` を出力し、`ai_model_ok=false` としてフェイルセーフを維持する。
...
| `models/` | ローカルのYOLOモデル重み置き場。期待パスは `models/yolo_bear.pt`。通常Gitには入れない。 |
...
1. 軽量YOLOモデルを `models/yolo_bear.pt` に配置する。
2. カメラを確認する。
   ```bash
   python3 raspberry_pi/camera_ai/camera_test.py --device /dev/video0
   ```
